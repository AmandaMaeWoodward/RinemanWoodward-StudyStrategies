...
```{r}
StudyDat  <-read.csv("~/Desktop/Study Strategies Project/Study Strategies Fixed.csv")
...
ggplot(PsGraph, aes(Strategies, fill=MC1_PriorStrategyUtility))+ geom_bar() + theme_classic()+ scale_fill_manual(name = "Prior Strategy Utility", values = c("#CAA494", "#9BB8B3", "#305252"))+ theme(legend.position="none") + theme(axis.text.x = element_text(size = 12, angle = 90, vjust = 0.5, hjust=1)) +facet_wrap(~MC1_PriorStrategyUtility, scales = "free_x") + ggtitle("Time Point One") + xlab(label = "Study Strategies") + ylab(label ="Prior Use of Study Strategies") + theme(plot.title = element_text(size = 10)) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.title = element_text(size = 12, hjust = 0.5)) 
...
ggplot(UsGraph, aes(Strategies, fill=MC2_UsedUtility))+ geom_bar() + theme_classic()+ scale_fill_manual(values = c("#CAA494", "#9BB8B3", "#305252"))+facet_wrap(~MC2_UsedUtility)+theme(axis.text.x = element_text(size= 12, angle = 90, vjust = 0.5, hjust=1)) + ggtitle("Time Point Two") + theme(legend.position="none") + xlab(label = "Study Strategies") + ylab(label ="Student Use of Study Strategies") + theme(plot.title = element_text(size = 12, hjust = 0.5))                           
...
binom.test(38, (38+28))
```
**Note**: Students are not using high utility strategies more often than chance at time point 2. 
...
MC2StudyDat$HighImpact<- ifelse(MC2StudyDat$OverallUtility==1 & MC2StudyDat$lowImpact !=1, 1, 0)
summary(as.factor(MC2StudyDat$HighImpact))

binom.test(20,66)

#students are choosing high utility strategies LESS OFTEN THAN CHANCE at time point two. 
...
binom.test(18,66)
 #students are using low utility strategies less often than chance at time point two.
```
...
class(MC3_CurrentLongDat$MC3_UsedUtility)
MC3_CurrentLongDat$MC3_UsedUtility<-ordered(MC3_CurrentLongDat$MC3_UsedUtility, levels= c("Low", "Moderate", "High"))
```
...
ggplot(PsGraph, aes(Strategies, fill=MC3_UsedUtility))+ geom_bar() + theme_classic()+ scale_fill_manual(values = c("#CAA494", "#9BB8B3", "#305252"))+facet_wrap(~MC3_UsedUtility)+theme(axis.text.x = element_text(size = 12, angle = 90, vjust = 0.5, hjust=1)) + ggtitle("Time Point Three") + theme(legend.position = "none") + xlab(label = "Study Strategies") + ylab(label ="Student Use of Study Strategies") + theme(plot.title = element_text(size = 12, hjust = 0.5))                           
```
...
binom.test(23, 66)
```
#students are choosing high utility less often than chance at time point 3
```{r}
...
#students are statistically choosing low utility at time point three.
...
```{r}
class(MC3StudyDat$OverallUtility)
MC3StudyDat$Final.Grade
summary(is.na(MC3StudyDat$Final.Grade))

cor.test(MC3StudyDat$JustHighImpact, MC3StudyDat$Final.Grade, use = "complete.obs", method = "spearman")

cor.test(MC3StudyDat$JustLowImpact, MC3StudyDat$Final.Grade, use = "complete.obs", method = "spearman")
...
cor.test(MC2StudyDat$MC2_ActualGrade,MC2StudyDat$MC_2AnticipatedGrade, method= "spearman", use = "complete.obs")

cor.test(MC3StudyDat$MC3_ActualGrade,MC3StudyDat$MC3_AnticipatedGrade, method= "spearman", use = "complete.obs")
```
...
```{r}
summary(as.factor(MC3StudyDat$MC3_Usefulness))
binom.test((16+38), 66)
```

We hypothesize that students who use effective strategies will have higher overall grades in the course. 
#Results: Assuming I have this correct, then students grades on the exam did not correlate with high or low impact strategies on exam 2 and exam 3. 

```{r}
levels(MC2StudyDat$MC2_ActualGrade)
mean(na.omit(MC2StudyDat$MC2_ActualGrade))
mean(na.omit(MC3StudyDat$MC3_ActualGrade))
mean(na.omit(StudyDat$Final.Exam.Grades))
```

We hypothesize that students who use effective strategies will have higher grades than students who use ineffective strategies. 
#Results: students exam grades were not correlated with their study strategies. 
```{r Study Strategy and grades on exam 2 and 3}
#Time Point two high utility study strategies and exam grade not correlated 
cor.test(MC2StudyDat$MC2_ActualGrade, MC2StudyDat$HighImpact, use= "complete.obs")
#Time Point two low utility study strategies and exam grade not correlated 
cor.test(MC2StudyDat$MC2_ActualGrade, MC2StudyDat$lowImpact, use= "complete.obs")

#Time point three high utility and exam grade not correlated 
cor.test(MC3StudyDat$MC3_ActualGrade, MC3StudyDat$JustHighImpact, use= "complete.obs")

#Time point three low utility and exam grade not correlated 
cor.test(MC3StudyDat$MC3_ActualGrade, MC3StudyDat$JustLowImpact, use= "complete.obs")
```
We hypothesize that students who use effective strategies will have higher overall grades in the course. 
#Results: If I have this correct, there was no difference in the final grade between students who used and high versus low strategies. 
- The question here is, with the combined data is it using time point 2 exam grade or time point 3 exam grade or did it combine them? Update: it did not combine them, so I ran a test for time point two and final exam grade and time point 3 and final exam grade. 
- Also, I get two different results when I use chisq.test and summary(table), based on my online searches these should both work as chi square tests.
```{r chi square test}
combinedDat<- left_join(MC2StudyDat, MC3StudyDat)

#Is this how to compare two groups in a chi square test? I wasn't sure if the + would mean that the two groups were being compared or not. 
chiTestMC2 <- chisq.test(table(combinedDat$FInal.Exam.Letter.grade,combinedDat$JustHighImpact + combinedDat$JustLowImpact))

summary(chiTestMC2)

summary(table(combinedDat$Final.Grade,combinedDat$JustHighImpact + combinedDat$JustLowImpact))

#the summaries below are separated by just high impact and just low impact, but I need to compare these two groups, so that is what I did above this one. 
summary(table(combinedDat$Final.Grade, combinedDat$JustHighImpact))
summary(table(combinedDat$Final.Grade, combinedDat$JustLowImpact))

summary(table(combinedDat$FInal.Exam.Letter.grade,combinedDat$lowImpact))
summary(table(combinedDat$FInal.Exam.Letter.grade,combinedDat$HighImpact))

summary(table(combinedDat$FInal.Exam.Letter.grade,combinedDat$HighImpact + combinedDat$lowImpact))
#this is using time point 2 

summary(table(combinedDat$FInal.Exam.Letter.grade, combinedDat$JustHighImpact + combinedDat$JustLowImpact))
#This one does not give a p-value
#this is using time point 3 
```
#Questions: 
  - Do the letter grades need to be put in frequency categories?
  - To assess whether students who use high utility have higher overall grades do I need to use MC2 used utility or MC3 used utility? 
    - I think we needed to determine if the use of strategies was stable? Do we need to do a within person analysis, such as an ANOVA?
    -To do this, I believe I need to have long data where the used strategy for time point 2 and 3 need to be combined. 
    
```{r}
stableStrategy<- t.test(paired= TRUE)
```
